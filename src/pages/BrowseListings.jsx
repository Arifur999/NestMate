...
import { FaArrowRight, FaMapMarkerAlt, FaMoneyBillWave, FaBed, FaCheckCircle } from 'react-icons/fa';
        ...
        <table className="min-w-full text-sm md:text-base text-left text-gray-700 border border-gray-200">
          <thead className="bg-indigo-600 text-white">
            <tr>
              ...
              <th className="py-3 px-4"><FaMapMarkerAlt className="inline mr-1" /> Location</th>
              <th className="py-3 px-4"><FaMoneyBillWave className="inline mr-1" /> Rent</th>
              <th className="py-3 px-4"><FaBed className="inline mr-1" /> Room Type</th>
              <th className="py-3 px-4"><FaCheckCircle className="inline mr-1" /> Availability</th>
          ...
          <tbody>
            {roommates.map((rm) => (
              ...
              >
                <td className="py-3 px-4 font-medium text-indigo-800">{rm.title}</td>
                <td className="py-3 px-4">{rm.location}</td>
                <td className="py-3 px-4">${rm.rent}</td>
                <td className="py-3 px-4">{rm.roomType}</td>
                <td
                  className={`py-3 px-4 font-semibold ${
                    rm.availability === 'available' ? 'text-green-600' : 'text-red-500'
                  }`}
                >
                  {rm.availability}
                </td>
